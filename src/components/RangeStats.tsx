...
export default function RangeStats() {
  ...
  const tiles = [
    ...
    // click → tab Đơn hàng, filter reported_all cùng khoảng sự kiện nên số khớp nhau
    {
      label: "Đơn lỗi",
      value: data?.reported,
      icon: ShieldAlert,
      color: "text-amber-600 bg-amber-50",
      href: `/orders?status=reported_all&efrom=${range.from}&eto=${range.to}`,
    },
      ...
      <div className="grid grid-cols-3 gap-3">
        {tiles.map((t) => {
          ...
          return "href" in t && t.href ? (
            <Link key={t.label} href={t.href} className={`${cls} active:scale-[0.98] hover:border-[#f1592a]/30 transition`}>
              {inner}
            </Link>
          ) : (
